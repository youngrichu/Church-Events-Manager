...
class RSVPManager {
    ...
    public function create_rsvp($event_id, $user_id, $status = 'attending') {
        ...
        if ($event_meta && $event_meta->max_attendees > 0) {
            $current_attendees = $this->get_attendee_count($event_id);
            if ($current_attendees >= $event_meta->max_attendees) {
                return new \WP_Error('event_full', 'Event has reached maximum capacity');
            }
        }
    ...
    public function get_rsvp_status($event_id, $user_id) {
        ...
        return [
            ...
            'max_attendees' => $event_meta ? $event_meta->max_attendees : 0,
            'is_full' => $event_meta && $event_meta->max_attendees > 0 && 
                        $attendee_count >= $event_meta->max_attendees
        ];
    }
} 